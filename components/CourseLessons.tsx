...
export default function CourseLessons({ course }: { course: Course }) {
  ...
  useEffect(() => { void supabase.auth.getUser().then(({ data }) => { if (!data.user) router.replace(`/login?next=/cursos/${course.slug}`); else setUserId(data.user.id); }); }, [course.slug, router]);
  ...
  function mark(id: string) { if (!key || watched.includes(id)) return; const next = [...watched, id]; setWatched(next); window.localStorage.setItem(key, JSON.stringify(next)); }
  ...
  return <div className="mt-8 flex flex-col gap-4">
    <div className="border border-blue-200 bg-blue-50 p-5"><div className="flex justify-between"><b className="font-display text-ink">Tu progreso</b><span className="font-mono text-blue-700">{Math.round(done / ids.length * 100)}%</span></div><div className="mt-3 h-2 bg-blue-100"><div className="h-full bg-blue-600" style={{ width: `${done / ids.length * 100}%` }} /></div><p className="mt-2 text-xs text-muted">{done} de {ids.length} clases completadas</p>{complete && <button onClick={() => router.push(`/certificado/${course.slug}`)} className="mt-4 rounded-cell bg-blue-600 px-4 py-2 text-sm font-bold text-white">Descargar certificado PDF</button>}</div>
    {course.modules.map((module, moduleIndex) => <section key={module.title} className="data-cell overflow-hidden"><div className="border-b border-line bg-blue-50 px-5 py-4"><p className="font-mono text-xs uppercase tracking-[0.14em] text-blue-700">Módulo {String(moduleIndex + 1).padStart(2, "0")}</p><h3 className="mt-1 font-display text-xl font-bold text-ink">{module.title}</h3></div><div className="divide-y divide-line">{module.lessons.map((lesson, lessonIndex) => { const id = `${module.title}:${lesson.title}`; const isDone = watched.includes(id); return <article key={lesson.title} className="px-5 py-4"><h4 className="font-display font-bold text-ink">Clase {lessonIndex + 1}: {lesson.title}</h4>{lesson.videoUrl ? <video controls preload="metadata" src={lesson.videoUrl} onEnded={() => mark(id)} className="mt-4 aspect-video w-full rounded-cell bg-ink" /> : <div className="mt-4 flex aspect-video items-center justify-center rounded-cell bg-blue-950 text-sm text-blue-100">Video de esta clase próximamente</div>}<ul className="mt-3 grid gap-3 text-sm text-muted md:grid-cols-2">{lesson.topics.map((topic) => <li key={topic}><strong className="text-ink">{topic}:</strong> {getTopicConcept(topic)}</li>)}</ul>{lesson.videoUrl && <button disabled={isDone} onClick={() => mark(id)} className="mt-4 rounded-cell border border-blue-300 px-4 py-2 text-sm text-blue-700 disabled:text-green-700">{isDone ? "Vista" : "Marcar clase como vista"}</button>}</article>; })}</div></section>)}
  </div>;
}
