...
export default function CourseLessons({ course }: { course: Course }) {
  ...
  return <div className="mt-8 flex flex-col gap-4">
    ...
    {course.modules.map((module, moduleIndex) => <section key={module.title} className="data-cell overflow-hidden"><div className="border-b border-line bg-blue-50 px-5 py-4"><p className="font-mono text-xs uppercase tracking-[0.14em] text-blue-700">Módulo {String(moduleIndex + 1).padStart(2, "0")}</p><h3 className="mt-1 font-display text-xl font-bold text-ink">{module.title}</h3></div><div className="divide-y divide-line">{module.lessons.map((lesson, lessonIndex) => { const id = `${module.title}:${lesson.title}`; const isDone = watched.includes(id); return <article key={lesson.title} className="px-5 py-4"><h4 className="font-display font-bold text-ink">Clase {lessonIndex + 1}: {lesson.title}</h4><p className="mt-2 text-sm text-muted">Sigue esta ruta: revisa el video, descarga el material y resuelve la práctica para completar la clase.</p>{lesson.videoUrl ? <video controls preload="metadata" src={lesson.videoUrl} onEnded={() => mark(id)} className="mt-4 aspect-video w-full rounded-cell bg-ink" /> : <div className="mt-4 flex aspect-video items-center justify-center rounded-cell bg-blue-950 text-sm text-blue-100">El video de esta clase estará disponible próximamente.</div>}<ul className="mt-3 grid gap-3 text-sm text-muted md:grid-cols-2">{lesson.topics.map((topic) => <li key={topic}><strong className="text-ink">{topic}:</strong> {getTopicConcept(topic)}</li>)}</ul><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => downloadMaterial(lesson.title, lesson.topics)} className="rounded-cell border border-blue-300 px-4 py-2 text-sm font-medium text-blue-700 hover:bg-blue-50">Descargar material</button></div><section className="mt-5 border-l-4 border-accent bg-blue-50 p-4"><p className="data-cell-header">Práctica aplicada</p><h5 className="mt-1 font-display font-bold text-ink">Aplica {lesson.topics.join(", ")} en un caso real</h5><ol className="mt-3 list-decimal space-y-1 pl-5 text-sm text-muted"><li>Usa el material descargado o crea datos de un negocio, proyecto o actividad personal.</li><li>Desarrolla una solución usando los conceptos de esta clase.</li><li>Registra qué hiciste, qué resultado obtuviste y qué mejorarías.</li></ol><label className="mt-4 block text-sm font-medium text-ink" htmlFor={`practice-${course.slug}-${moduleIndex}-${lessonIndex}`}>Tu respuesta o evidencia</label><textarea id={`practice-${course.slug}-${moduleIndex}-${lessonIndex}`} value={practiceNotes[id] ?? ""} onChange={(event) => setPracticeNotes((notes) => ({ ...notes, [id]: event.target.value }))} placeholder="Escribe aquí tus pasos, resultado o enlace a tu trabajo." className="mt-2 min-h-28 w-full rounded-cell border border-line bg-white p-3 text-sm text-ink placeholder:text-muted focus:border-accent focus:outline-none" /><button disabled={!isEnrolled || isDone || !practiceNotes[id]?.trim()} onClick={() => mark(id)} className="mt-3 rounded-cell bg-accent px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-ink disabled:cursor-not-allowed disabled:bg-line disabled:text-muted">{isDone ? "Práctica completada" : "Completar práctica y avanzar"}</button>{!isEnrolled && <p className="mt-2 text-xs text-muted">Inscríbete en el curso para registrar tu práctica y progreso.</p>}</section></article>; })}</div></section>)}
  </div>;
}
