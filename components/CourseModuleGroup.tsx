...
export default function CourseModuleGroup({ track, title, image, description, professionalUse, levels }: CourseModuleGroupProps) {
  ...
  return (
    <Link
      href={isPythonTrack ? "/cursos/python-basico" : `/cursos/modulo/${track}`}
      ...
      aria-label={isPythonTrack ? "Ingresar directamente al curso Python Básico" : `Ingresar al módulo de ${title}`}
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-5 sm:p-6">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-cell bg-base px-2 py-1 text-xs font-medium text-muted">{isPythonTrack ? "Acceso directo" : `${totalDuration} niveles`}</span>
        </div>
        ...
        <div className="mt-auto flex flex-wrap items-center justify-between gap-2 pt-2">
          <span className="text-base font-medium text-accent group-hover:underline sm:text-sm">{isPythonTrack ? "Ver curso →" : "Ver módulo →"}</span>
        </div>
      </div>
    </Link>
  );
