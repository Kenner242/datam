      ...
      <main className="detective-panel">
        ...
        <div hidden={mode === "ciencia"}>
          {!selected ? <div className="detective-empty"><BookOpen className="mx-auto h-10 w-10 text-blue-300" /><h2 className="mt-3 font-display text-xl font-bold text-white">Tu caso aún no existe</h2><p className="mt-2 text-sm text-blue-100">Sube un material para iniciar tu investigación cognitiva.</p></div> : <>
            ...
            <DetectiveMode mode={mode} selected={selected} concepts={concepts} sentences={sentences} hunterSentence={hunterSentence} hunterDone={hunterDone} feynmanText={feynmanText} setFeynmanText={setFeynmanText} daxFeedback={daxFeedback} daxLoading={daxLoading} feynmanRubric={feynmanRubric} caseAnswers={caseAnswers} setCaseAnswers={setCaseAnswers} onFeynman={evaluateFeynman} onHunter={answerHunter} onNextHunter={() => { setHunterIndex((index) => index + 1); setHunterDone(false); }} onCase={completeCase} state={state} />
          </>}
        </div>
      </main>
    </div>
...
function DetectiveMode({ mode, selected, concepts, sentences, hunterSentence, hunterDone, feynmanText, setFeynmanText, daxFeedback, daxLoading, feynmanRubric, caseAnswers, setCaseAnswers, onFeynman, onHunter, onNextHunter, onCase, state }: { mode: Mode; selected: StoredLibraryFile; concepts: Concept[]; sentences: string[]; hunterSentence: string; hunterDone: boolean; feynmanText: string; setFeynmanText: (value: string) => void; daxFeedback: string; daxLoading: boolean; feynmanRubric: string[]; caseAnswers: string[]; setCaseAnswers: (value: string[]) => void; onFeynman: () => void; onHunter: (isFalse: boolean) => void; onNextHunter: () => void; onCase: () => void; state: DetectiveState }) {
  if (mode === "mapa") return <section className="detective-content"><p className="data-cell-header">Mapa conceptual basado en el material</p><h3>De la fuente a sus ideas y evidencias</h3><p>La jerarquía se construye a partir de conceptos detectados y fragmentos textuales reales; las citas sirven para comprobar cada relación.</p>{concepts.length ? <div className="concept-tree"><div className="concept-tree-root"><BookOpen className="h-4 w-4" /><span>{selected.name}</span></div><div className="concept-tree-branches">{concepts.slice(0, 8).map((concept, index) => <article key={concept.term} className="concept-branch"><div className={`concept-tree-concept concept-level-${index % 3}`}><b>{concept.term}</b><small>{concept.frequency} apariciones</small></div><div className="concept-tree-evidence"><span className="evidence-label">Evidencia del material</span><p>{concept.context || sentences.find((sentence) => sentence.toLowerCase().includes(concept.term.toLowerCase())) || "El texto detecta el término, pero no hay una oración legible para mostrar como evidencia."}</p></div></article>)}</div></div> : <div className="research-empty">No se encontró texto suficiente para construir un mapa conceptual. Prueba con un PDF con texto seleccionable o un archivo de texto.</div>}</section>;
