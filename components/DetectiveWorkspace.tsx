...
function DetectiveMode({ mode, selected, concepts, conceptMap, selectedConcept, mapView, onMapView, onSelectConcept, sentences, hunterSentence, hunterDone, feynmanText, setFeynmanText, daxFeedback, daxLoading, feynmanRubric, caseAnswers, setCaseAnswers, onFeynman, onHunter, onNextHunter, onCase, state }: { mode: Mode; selected: StoredLibraryFile; concepts: Concept[]; conceptMap: ConceptMap | null; selectedConcept: ConceptNode | null; mapView: "tree" | "outline"; onMapView: (view: "tree" | "outline") => void; onSelectConcept: (id: string) => void; sentences: string[]; hunterSentence: string; hunterDone: boolean; feynmanText: string; setFeynmanText: (value: string) => void; daxFeedback: string; daxLoading: boolean; feynmanRubric: string[]; caseAnswers: string[]; setCaseAnswers: (value: string[]) => void; onFeynman: () => void; onHunter: (isFalse: boolean) => void; onNextHunter: () => void; onCase: () => void; state: DetectiveState }) {
  if (mode === "mapa") return <section className="detective-content"><div className="concept-map-heading"><div><p className="data-cell-header">Mapa conceptual del documento</p><h3>Ideas ordenadas con evidencia</h3></div><div className="concept-map-switch" role="tablist" aria-label="Presentación del mapa"><button type="button" role="tab" aria-selected={mapView === "tree"} onClick={() => onMapView("tree")}>Mapa</button><button type="button" role="tab" aria-selected={mapView === "outline"} onClick={() => onMapView("outline")}>Esquema</button></div></div><p>Organiza la fuente en secciones, conceptos subordinados y citas. Selecciona un concepto para revisar la evidencia textual que lo respalda.</p>{conceptMap?.warnings.map((warning) => <p key={warning} role="status" className="concept-map-warning">{warning}</p>)}{conceptMap?.concepts.length ? <><div className="concept-map-root"><BookOpen className="h-4 w-4" /><span>{conceptMap.documentTitle}</span><small>{conceptMap.wordCount.toLocaleString("es-PE")} palabras · {conceptMap.sections.length} secciones</small></div>{mapView === "tree" ? <div className="concept-map-tree">{conceptMap.concepts.map((section, index) => <article key={section.id} className={`map-section map-section-${index % 3}`}><button type="button" className="map-section-title" onClick={() => section.children[0] && onSelectConcept(section.children[0].id)}><span className="map-level-label">{`Sección ${index + 1}`}</span><b>{section.label}</b><small>{section.children.length} conceptos</small></button><div className="map-concept-list">{section.children.map((node) => <button type="button" key={node.id} onClick={() => onSelectConcept(node.id)} className={`map-concept-node ${selectedConcept?.id === node.id ? "selected" : ""}`}><b>{node.label}</b><small>{node.frequency} menciones</small></button>)}</div>{section.evidence[0] && <p className="map-section-evidence"><span>Evidencia de sección</span>{section.evidence[0]}</p>}</article>)}</div> : <div className="concept-map-outline">{conceptMap.concepts.map((section, index) => <details key={section.id} open={index === 0}><summary><span>{`0${index + 1}`}</span><b>{section.label}</b><small>{section.children.length} conceptos</small></summary><ol>{section.children.map((node) => <li key={node.id}><button type="button" onClick={() => onSelectConcept(node.id)}>{node.label}</button><ul>{node.evidence.map((evidence, evidenceIndex) => <li key={`${node.id}-${evidenceIndex}`}>{evidence}</li>)}</ul></li>)}</ol></details>)}</div>}<aside className="concept-evidence-panel"><div className="concept-evidence-heading"><span>Concepto y respaldo textual</span>{selectedConcept && <b>{selectedConcept.label}</b>}</div>{selectedConcept?.evidence.length ? selectedConcept.evidence.map((evidence, index) => <blockquote key={`${selectedConcept.id}-${index}`>“{evidence}”</blockquote>) : <p>{selectedConcept ? "No se encontró una oración legible para citar este concepto." : "Selecciona un concepto del mapa para consultar sus citas."}</p>}</aside></> : <div className="concept-map-empty"><BookOpen className="mx-auto h-7 w-7" /><b>No se pudo construir el mapa</b><p>Este documento no contiene suficiente texto estructurado. En PDF escaneados se necesita OCR; los mapas no inventan conceptos sin evidencia.</p></div>}</section>;
