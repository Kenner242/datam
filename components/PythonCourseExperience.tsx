...
type PythonStage = { id: string; title: string; icon: string; goal: string; color: string; keywords: string[] };
type PracticeState = { code: string; output: string; checked: boolean; passed: boolean };

const STAGES: PythonStage[] = [
  { id: "fundamentos", title: "Introducción a Python", icon: "🐍", goal: "Entender qué es Python y guardar información en variables.", color: "#60a5fa", keywords: ["print", "=", "nombre"] },
  { id: "decisiones", title: "Operadores y decisiones", icon: "🎯", goal: "Hacer que tus programas tomen decisiones según los datos.", color: "#3b82f6", keywords: ["if", "elif", "else"] },
  { id: "automatizacion", title: "Bucles y automatización", icon: "🔄", goal: "Repetir tareas sin escribir el mismo código muchas veces.", color: "#4ade80", keywords: ["for", "while", "range"] },
  { id: "funciones", title: "Funciones", icon: "🧩", goal: "Reutilizar lógica y construir soluciones mantenibles.", color: "#16a34a", keywords: ["def", "return"] },
  { id: "datos", title: "Estructuras de datos", icon: "📚", goal: "Guardar, recorrer y analizar colecciones de información.", color: "#a78bfa", keywords: ["list", "dict", "len"] },
  { id: "proyecto", title: "Proyecto aplicado", icon: "🚀", goal: "Integrar todo lo aprendido en un analizador de ventas.", color: "#7c3aed", keywords: ["sum", "max", "min", "f"] },
];
...
const STARTERS = [
  ...
  'ventas = [120, 340, 90, 500, 210]\ntotal = sum(ventas)\nprint(f"Total: {total}")\nprint(f"Mejor día: {max(ventas)}")\nprint(f"Peor día: {min(ventas)}")',
];
...
export default function PythonCourseExperience({ course }: { course: Course }) {
  ...
  useEffect(() => {
    try {
      ...
      setCompleted(Array.isArray(saved) ? saved.filter((id): id is string => typeof id === "string" && validIds.has(id)) : []);
    } catch {
      setCompleted([]);
    }
  }, [storageKey]);
  ...
  function selectStage(index: number) {
    ...
    if (!unlocked) { setMessage("Completa el módulo anterior para desbloquear esta ruta."); return; }
    setActiveStage(index); setMessage(""); setPractice({ code: STARTERS[index], output: "", checked: false, passed: false });
  }

  function completeStage() {
    if (!practice.passed || completed.includes(STAGES[activeStage].id)) return;
    const next = Array.from(new Set([...completed, STAGES[activeStage].id]));
    setCompleted(next);
    try {
      window.localStorage.setItem(storageKey, JSON.stringify(next));
      setMessage(`Módulo completado: ${STAGES[activeStage].title}.`);
    } catch {
      setMessage("Módulo completado en esta sesión, pero no se pudo guardar el progreso en este dispositivo.");
    }
  }
  ...
  function runPractice() {
    ...
    setMessage(passed ? "Práctica superada. Ya puedes completar el módulo." : "Revisa el código e inténtalo de nuevo.");
  }

  return <section className="python-experience" aria-label="Experiencia interactiva de Python Básico">
    <header className="python-experience-head"><div><p className="python-kicker">DataM AI · Python Básico</p><h2>Mi ruta profesional en DataM</h2><p>Aprende, practica y crea un producto real con Python. Cada módulo combina lectura, laboratorio, retroalimentación y evidencia.</p></div><div className="python-experience-stats"><b>{progress}%</b><span>ruta completada</span><small>{completedStages}/{STAGES.length} módulos</small></div></header>
    <div className="python-stage-route" role="list" aria-label="Ruta de Python"><div className="python-stage-route-line" aria-hidden="true" />{STAGES.map((stage, index) => { const isDone = completed.includes(stage.id); const isUnlocked = index === 0 || completed.includes(STAGES[index - 1].id); return <button key={stage.id} type="button" role="listitem" className={`python-route-stage ${activeStage === index ? "active" : ""} ${isDone ? "done" : ""} ${!isUnlocked ? "locked" : ""}`} onClick={() => selectStage(index)}><span style={{ backgroundColor: isDone ? "#16a34a" : isUnlocked ? stage.color : "#64748b" }}>{isDone ? <Check className="h-5 w-5" /> : !isUnlocked ? <LockKeyhole className="h-4 w-4" /> : index + 1}</span><b>{stage.icon} {stage.title}</b><small>{isDone ? "Completado" : isUnlocked ? "Disponible" : "Bloqueado"}</small></button>; })}</div>
    {message && <p role="status" className="python-experience-message">{message}</p>}
    <div className="python-stage-card"><div className="python-stage-card-icon" style={{ backgroundColor: STAGES[activeStage].color }}>{STAGES[activeStage].icon}</div><div className="min-w-0 flex-1"><p className="python-stage-label">Módulo {activeStage + 1} de {STAGES.length}</p><h3>{STAGES[activeStage].title}</h3><p>{STAGES[activeStage].goal}</p><div className="python-topic-tags">{STAGES[activeStage].keywords.map((keyword) => <span key={keyword}>{keyword}</span>)}</div></div><button type="button" onClick={completeStage} disabled={!practice.passed || completed.includes(STAGES[activeStage].id)} className="python-complete-button"><CheckCircle2 className="h-4 w-4" /> {completed.includes(STAGES[activeStage].id) ? "Módulo completado" : practice.passed ? "Completar módulo" : "Aprueba la práctica"}</button></div>
    <div className="python-experience-grid"><section className="python-lessons-panel"><div className="python-panel-heading"><div><p className="python-section-label">Contenido conectado</p><h3>Clases del módulo</h3></div><span>{lessons.length} clase{lessons.length === 1 ? "" : "s"}</span></div>{lessons.map((lesson) => <button key={lesson.title} type="button" onClick={() => setOpenLesson(lesson)} className="python-lesson-card"><span>{lesson.content ? "📖" : "🧪"}</span><span className="min-w-0 flex-1 text-left"><b>{lesson.title}</b><small>{lesson.topics.join(" · ")}</small></span><ChevronRight className="h-4 w-4" /></button>)}</section><section className="python-lab-panel"><div className="python-panel-heading"><div><p className="python-section-label">Práctica inmediata</p><h3><FlaskConical className="inline h-5 w-5" /> Laboratorio</h3></div><span>sin instalar nada</span></div><p className="python-lab-prompt">Escribe o modifica el código del módulo. El revisor comprueba la estructura y te indica qué mejorar.</p><textarea value={practice.code} onChange={(event) => setPractice((current) => ({ ...current, code: event.target.value, checked: false }))} className="python-code-editor" spellCheck={false} aria-label="Editor de práctica Python" /><div className="python-lab-actions"><button type="button" onClick={runPractice} className="python-run-button"><Code2 className="h-4 w-4" /> Revisar código</button><button type="button" onClick={() => setPractice({ code: STARTERS[activeStage], output: "", checked: false, passed: false })} className="python-reset-button"><RotateCcw className="h-4 w-4" /> Reiniciar</button></div>{practice.checked && <div className={`python-output ${practice.passed ? "passed" : "needs-work"}`}>{practice.output}</div>}</section></div>
